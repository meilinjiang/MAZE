...
trans.zipm <- function(dat, theta, K, xval, num_Z, zval) {
    ...
    if (K == 1) {
        ...
        psi_k <- c(theta[6 + num_Z + (2 + num_Z) * K + 1:(K - 1)], 1 -
            sum(theta[6 + num_Z + (2 + num_Z) * K + 1:(K - 1)]))
    }
    ...
    theta_trans <- list(beta0 = beta0, beta1 = beta1, beta2 = beta2,
        beta3 = beta3, beta4 = beta4, delta = delta, eta = eta, beta_T_Z = beta_T_Z,
        alpha_k = alpha_k, loglambda_ik = loglambda_ik, lambda_ik = lambda_ik,
        psi_k = psi_k, psi_ik = psi_ik, Delstar_i = Delstar_i, Del_i = Del_i,
        log_em1 = log_em1)
    return(theta_trans)
}
...
bounds.zipm <- function(dat, K, group, num_Z) {
    ...
    if (K == 1) {
        if (group == 1) {
            ...
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, 2 + num_Z + 2 +
                num_Z), 1e-06)
        }
    } else {
        if (group == 1) {
            ui1 <- diag(rep(1, s + 1 + (2 + num_Z) * K + (K - 1)))
            ui2 <- diag(c(rep(0, s + 1 + (2 + num_Z) * K), rep(-1, K -
                1)))
            ...
            ui1 <- diag(rep(1, s + 2 + (2 + num_Z) * K + (K - 1) + 2 +
                num_Z))
            ui2 <- diag(c(rep(0, s + 1 + (2 + num_Z) * K), rep(-1, K -
                1), rep(0, (2 + num_Z) + 1)))
            ui <- rbind(ui1, ui2[s + 1 + (2 + num_Z) * K + 1:(K - 1),
...
ComputeInit2.zipm <- function(dat, K, num_Z, Z_names, B, limits, explicit = T) {
    ...
    while (euclidean_dist(init, init2) > limits) {
        ...
        m <- constrOptim(init, function(x) negQ2_G1(dat, x, K, num_Z,
            Z_names, B, tau), grad = NULL, ui = bd$ui, ci = bd$ci, outer.iterations = 500,
...
effects.zipm <- function(dat, theta, x1, x2, K, num_Z, zval, calculate_se = F,
    ...
    if (calculate_se == T) {
        ...
        g_NIE2 <- c(-diff(Del_x12), x2 * (-diff(Del_x12)), g_NIE2_alpha_k,
            (theta_trans[["beta2"]] + theta_trans[["beta4"]] * x2) *
                (-1) * diff((1 - Delstar_x12) * (exp(-theta_trans[["lambda_ik"]])[,
        ...
        g_NIE <- c(g_NIE1[1], g_NIE2[1:2], g_NIE1[1 + 1:((2 + num_Z) *
            (K + 1) + K - 1)] + g_NIE2[2 + 1:((2 + num_Z) * (K + 1) +
            K - 1)])

        if (Group1) {
            g_NIE1 <- g_NIE <- g_NIE1[1:((2 + num_Z) * K + K - 1 + 1)]
            V1 <- V3 <- vcovar[c(2, 4 + num_Z + 1:((2 + num_Z) * K +
                K - 1)), c(2, 4 + num_Z + 1:((2 + num_Z) * K + K - 1))]
            NIE2_se <- NA
        } else {
            V1 <- vcovar[c(2, 6 + num_Z + 1:((2 + num_Z) * (K + 1) +
                K - 1)), c(2, 6 + num_Z + 1:((2 + num_Z) * (K + 1) +
                K - 1))]
            V2 <- vcovar[c(3, 5, 6 + num_Z + 1:((2 + num_Z) * (K + 1) +
                K - 1)), c(3, 5, 6 + num_Z + 1:((2 + num_Z) * (K + 1) +
                K - 1))]
            V3 <- vcovar[c(2, 3, 5, 6 + num_Z + 1:((2 + num_Z) * (K +
                1) + K - 1)), c(2, 3, 5, 6 + num_Z + 1:((2 + num_Z) *
                (K + 1) + K - 1))]
