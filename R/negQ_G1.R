...
negQ_G1.zilonm <- function(dat, theta, K, num_Z,
    ...
    calculate_ll = F) {
    # beta0-5, alpha0k, alpha1k, xi0, w0k,
    # gamma0, gamma1, eta, delta
    ...
    if (!calculate_tau) {
        ...
        l1_ik <- -log(theta_trans[["delta"]]) +
            log(1 - pf0) - log(M_group1) - log(2 *
            pi) - log(theta_trans[["sig"]]) - (log(M_group1) -
        ...
        if (!calculate_ll) {
            ...
            # need EM), but hessian can l1_i
            # <- log( bigpsi1*exp(l1_ik1) +
            # bigpsi2*exp(l1_ik2) )
...
negQ_G1.zipm <- function(dat, theta, K, num_Z,
    ...
    if (!calculate_tau) {
        ...
        l1_ik <- -log(theta_trans[["delta"]]) +
            ...
            theta_trans[["beta2"]] - (theta_trans[["beta3"]] +
            theta_trans[["beta4"]]) * X_group1 -
            theta_trans[["beta_T_Z"]])^2/(2 * theta_trans[["delta"]]^2)
