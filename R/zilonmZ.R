...
trans.zilonm <- function(dat, theta, K, xval, num_Z, zval) {
    ...
    theta_trans <- list(beta0 = beta0, beta1 = beta1, beta2 = beta2,
        ...
        psi_k = psi_k, eta = eta, beta_T_Z = beta_T_Z, mu_ik = mu_ik,
...
bounds.zilonm <- function(dat, K, group, num_Z) {
    ...
    if (K == 1) {
        if (group == 1) {
            ...
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, 3 + num_Z + 2 +
                num_Z), 1e-06)
        }
    } else {
        if (group == 1) {
            ...
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 + num_Z) * K +
                1), rep(1e-06, K - 1), rep(-(1 - 1e-06), K - 1), -(1 -
                1e-06))
        } else {
            ...
            ui <- rbind(ui1, ui2[s + 2 + (2 + num_Z) * K + 1:(K - 1),
                ...
                rep(0, (2 + num_Z) + 1)))
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 + num_Z) * K +
                1), rep(1e-06, K - 1), rep(-1000, 2 + num_Z), 1e-06,
...
ComputeInit2.zilonm <- function(dat, K, num_Z, Z_names, B, limits, explicit = T) {
    ...
    if (explicit) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            alpha_k <- matrix(init[4 + num_Z + 1:((2 + num_Z) * K)],
                ncol = K)
            ...
            delta <- sqrt(sum((Y_group1 - rowSums(k_to_ik(betas, n1) *
                desginMat_Y))^2)/n1)
...
effects.zilonm <- function(dat, theta, x1, x2, K, num_Z, zval, calculate_se = F,
    ...
    if (calculate_se == T) {
        ...
        g_NIE <- c(g_NIE1[1], g_NIE2[1:2], g_NIE1[1 + 1:((3 + num_Z) *
            K)], g_NIE1[(3 + num_Z) * K + 1 + 1:(2 + num_Z)] + g_NIE2[2 +
            1:(2 + num_Z)])

        if (Group1) {
            g_NIE1 <- g_NIE <- g_NIE1[1:((3 + num_Z) * K + 1)]
            V1 <- V3 <- vcovar[c(2, 4 + num_Z + 1:((3 + num_Z) * K)),
                c(2, 4 + num_Z + 1:((3 + num_Z) * K))]
            NIE2_se <- NA
        } else {
            V1 <- vcovar[c(2, 6 + num_Z + 1:((3 + num_Z) * K + 2 + num_Z)),
                c(2, 6 + num_Z + 1:((3 + num_Z) * K + 2 + num_Z))]
            V2 <- vcovar[c(3, 5, 6 + num_Z + (3 + num_Z) * K + 1:(2 +
                num_Z)), c(3, 5, 6 + num_Z + (3 + num_Z) * K + 1:(2 +
                num_Z))]
            V3 <- vcovar[c(2, 3, 5, 6 + num_Z + 1:((3 + num_Z) * K +
                2 + num_Z)), c(2, 3, 5, 6 + num_Z + 1:((3 + num_Z) *
                K + 2 + num_Z))]
...
weights = list(c(1.35817842745391e-12, 2.1431204556943e-07, 0.000266200513752717,
    ...
    c(1.16311658142558e-09, 1.19837013631707e-05, 0.00290251774790131,
        ...
        0.0108399371682559, 0.0134465366052857, 0.0165667862542476, 0.0202771838175001,
        0.0246610873147533, 0.0298086281173101, 0.0358165056041964, 0.0427876521577257,
        0.0508307575725705, 0.0600596423586363, 0.070592469906867, 0.0825507881107017,
        0.0960583918651895, 0.111239998988745, 0.128219733631201, 0.147119413257857,
        0.168056637948269, 0.191142684133427, 0.216480209117296, 0.24416077786984,
        0.274262229689068, 0.306845909417917, 0.341953795923017, 0.379605569386652,
        0.419795668445015, 0.462490398055368, 0.507625158831908, 0.555101878003633,
        0.604786730578404, 0.656508246131628, 0.710055901205469, 0.765179298908956,
        0.821588035266965, 0.878952345552782, 0.936904612745668, 0.995041804046133,
        1.05292887995527, 1.11010319396534, 1.16607986993243, 1.22035810957936,
        1.27242834553786, 1.32178011744377, 1.3679105116809, 1.41033297144626,
        1.44858625496132, 1.48224329788554, 1.51091972307417, 1.5342817381543,
        1.55205316984541, 1.56402140377323, 1.57004202927959), c(1.72376440360427e-12,
