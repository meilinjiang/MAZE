    ...
    if (explicit) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            sig2 <- sum(tau * (log(M_group1) - apply(alpha_k,
                2, function(t) rowSums(k_to_ik(t, n1) * desginMat_M)))^2)/n1
            xi0 <- log(sqrt(sig2))
            # beta02 ,beta1, beta34, beta5, beta_Z
            betas <- init[1:s]
            desginMat_Y <- cbind(1, dat_g1[, c("Mobs", "X")],
                dat_g1$X * dat_g1$Mobs, dat_g1[, Z_names])
            if (!XMint[2]) {
...
negQ_G2.zilonm <- function(dat, theta, K, num_Z, Z_names, B, tauG2 = NULL,
    ...
    l2_ik <- cbind(-log(theta_trans[["delta"]]) - (Y_group2 -
        theta_trans[["beta0"]] - theta_trans[["beta3"]] * X_group2 -
        theta_trans[["beta_T_Z"]])^2/(2 * theta_trans[["delta"]]^2) -
        0.5 * log(2 * pi), -log(theta_trans[["delta"]]) - theta_trans[["xi0"]] -
        log(2 * pi) + LogInt_k)
...
weights <- list(c(1.35817842745391e-12, 2.1431204556943e-07, 0.000266200513752717,
    0.0183431669899278, 0.230022394514789, 0.965976579412301,
    1.5707963267949), c(1.16311658142558e-09, 1.19837013631707e-05,
    0.00290251774790131, 0.0763857435708323, 0.531078275428054,
    1.38961475924726), c(4.93785387766319e-11, 1.86872822687364e-08,
    1.82633205937107e-06, 6.24825592407441e-05, 0.000949946804283469,
    0.00774260102606424, 0.0391750054936008, 0.137422107733168,
    0.360461418469344, 0.737437848361548, 1.19346302584916, 1.52328371863471),
    ...
    c(3.48419376702611e-12, 2.09893354045115e-11, 1.13060553474947e-10,
        ...
        1.27242834553786, 1.32178011744377, 1.3679105116809, 1.41033297144626,
    ...
    c(1.72376440360427e-12, 2.76085876713983e-12, 4.38886518997793e-12,
        ...
        1.4898932978833, 1.50423673806368, 1.51727027540505, 1.52895560835458,
