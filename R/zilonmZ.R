# Mediation analysis for zero-inflated log-normal mediators with
# confounders
...
trans.zilonm <- function(dat, theta, K, xval, num_Z, zval) {
    ...
    mu_ik <- apply(matrix(alpha_k, ncol = K), 2, function(t) rowSums(k_to_ik(t,
        n0) * designMat_M))
    Del_i <- expit(rowSums(k_to_ik(gammas, n0) * designMat_M))

    theta_trans <- list(beta0 = beta0, beta1 = beta1, beta2 = beta2, beta3 = beta3,
        beta4 = beta4, delta = delta, alpha_k = alpha_k, psi_k = psi_k,
        eta = eta, beta_T_Z = beta_T_Z, mu_ik = mu_ik, sig = sig, psi_ik = psi_ik,
        Del_i = Del_i)
    return(theta_trans)
}
...
negQ_G1.zilonm <- function(dat, theta, K, num_Z, Z_names, B = NULL, tauG1 = NULL,
    ...
    if (!calculate_tau) {
        ...
        l1_ik <- -log(theta_trans[["delta"]]) + log(1 - pf0) - log(M_group1) -
            log(2 * pi) - log(theta_trans[["sig"]]) - (log(M_group1) - theta_trans[["mu_ik"]])^2/(2 *
            theta_trans[["sig"]]^2) - (Y_group1 - theta_trans[["beta0"]] -
            theta_trans[["beta1"]] * M_group1 - theta_trans[["beta2"]] -
            (theta_trans[["beta3"]] + theta_trans[["beta4"]]) * X_group1 -
...
G1_init.zilonm <- function(dat, init, K, num_Z, Z_names) {
...
compare_mu.zilonm <- function(dat, init2, group, K, num_Z, Z_names, B = NULL) {
    if (group == 1) {
...
bounds.zilonm <- function(dat, K, group, num_Z) {
    ...
    if (K == 1) {
        if (group == 1) {
            ...
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, 3 + num_Z + 2 + num_Z),
                1e-06)
        }
    } else {
        if (group == 1) {
            ...
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 + num_Z) * K + 1),
...
ComputeInit2.zilonm <- function(dat, K, num_Z, Z_names, B, limits, explicit = T) {
    ...
    if (explicit) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            alpha_k <- matrix(init[4 + num_Z + 1:((2 + num_Z) * K)], ncol = K)
            desginMat_M <- cbind(1, dat_g1[, c("X", Z_names)])
            for (var in 1:nrow(alpha_k)) {
                var_k <- matrix(rep(desginMat_M[, var], K), ncol = K)
                alpha_k[var, ] <- colSums(tau * var_k * (log(M_group1) -
                  apply(alpha_k[-var, 1:K, drop = F], 2, function(t) rowSums(k_to_ik(t,
                    n1) * desginMat_M[, -var]))))/colSums(tau * var_k^2)
            }
            ...
            desginMat_Y <- cbind(1, dat_g1[, c("Mobs", "X", Z_names)])
            for (var in 1:length(betas)) {
                betas[var] <- sum((Y_group1 - rowSums(k_to_ik(betas[-var],
                  n1) * desginMat_Y[, -var])) * desginMat_Y[, var])/sum(desginMat_Y[,
                  var]^2)
            }
...
negQ_G2.zilonm <- function(dat, theta, K, num_Z, Z_names, B, tauG2 = NULL,
    calculate_tau = F, calculate_ll = F) {
    # group 2
    ...
    LogInt_k <- loghicpp_all(X_group2, Y_group2, theta_trans[["mu_ik"]],
        theta_trans[["sig"]], Ypart, theta_trans[["beta1"]], theta_trans[["delta"]],
    ...
    l2_ik <- cbind(-log(theta_trans[["delta"]]) - (Y_group2 - theta_trans[["beta0"]] -
        ...
        theta_trans[["delta"]]^2) - 0.5 * log(2 * pi), -log(theta_trans[["delta"]]) -
        log(theta_trans[["sig"]]) - log(2 * pi) + LogInt_k)
...
effects.zilonm <- function(dat, theta, x1, x2, K, num_Z, zval, calculate_se = F,
    ...
    if (calculate_se == T) {
        ...
        for (var in seq_len(2 + num_Z)) {
            g_NIE1_alpha_k[var, ] <- theta_trans[["beta1"]] * theta_trans[["psi_k"]] *
                diff((1 - Del_x12) * exp_x12 * desginMat[, var])
            g_NIE1_gammas[var] <- theta_trans[["beta1"]] * (-diff(Del_x12 *
                (1 - Del_x12) * m_x12 * desginMat[, var]))
            g_NIE2_gammas[var] <- (theta_trans[["beta2"]] + theta_trans[["beta4"]] *
                x2) * (-diff(Del_x12 * (1 - Del_x12) * desginMat[, var]))
        }
        # beta1, alpha0_k, alpha1_k, xi0, psi_k-1, gamma0, gamma1
        g_NIE1 <- c(diff((1 - Del_x12) * m_x12), g_NIE1_alpha_k, sig2 *
            NIE1, theta_trans[["beta1"]] * diff((exp_x12[, 1:(K - 1)] -
            exp_x12[, K]) * (1 - Del_x12)), g_NIE1_gammas)
        if (K == 1) {
            g_NIE1 <- g_NIE1[-(2 + num_Z + 3)]
        }

        # beta2, beta4, gamma0, gamma1
        g_NIE2 <- c(-diff(Del_x12), x2 * (-diff(Del_x12)), g_NIE2_gammas)

        g_NIE <- c(g_NIE1[1], g_NIE2[1:2], g_NIE1[1 + 1:((3 + num_Z) * K)],
            g_NIE1[(3 + num_Z) * K + 1 + 1:(2 + num_Z)] + g_NIE2[2 + 1:(2 +
                num_Z)])

        if (Group1) {
            g_NIE1 <- g_NIE <- g_NIE1[1:((3 + num_Z) * K + 1)]
            V1 <- V3 <- vcovar[c(2, 4 + num_Z + 1:((3 + num_Z) * K)), c(2,
                4 + num_Z + 1:((3 + num_Z) * K))]
            NIE2_se <- NA
        } else {
            V1 <- vcovar[c(2, 6 + num_Z + 1:((3 + num_Z) * K + 2 + num_Z)),
                c(2, 6 + num_Z + 1:((3 + num_Z) * K + 2 + num_Z))]
            V2 <- vcovar[c(3, 5, 6 + num_Z + (3 + num_Z) * K + 1:(2 + num_Z)),
                c(3, 5, 6 + num_Z + (3 + num_Z) * K + 1:(2 + num_Z))]
...
parameter_names.zilonm <- function(dat, K, num_Z, Z_names) {
    c(paste0("beta", c(0:4, Z_names)), "delta", paste0("alpha", rep(1:K,
        each = 2 + num_Z), rep(c(0:1, Z_names), K)), "xi0", paste0("psi",
...
weights = list(c(1.35817842745391e-12, 2.1431204556943e-07, 0.000266200513752717,
    ...
    c(1.16311658142558e-09, 1.19837013631707e-05, 0.00290251774790131, 0.0763857435708323,
        0.531078275428054, 1.38961475924726), c(4.93785387766319e-11, 1.86872822687364e-08,
        1.82633205937107e-06, 6.24825592407441e-05, 0.000949946804283469,
        0.00774260102606424, 0.0391750054936008, 0.137422107733168, 0.360461418469344,
        0.737437848361548, 1.19346302584916, 1.52328371863471), c(8.6759314149796e-12,
        ...
        0.0001320523412561, 0.000513393824067903, 0.00169087399814264, 0.00481629814392846,
        ...
        0.00375425097743183, 0.00613003763208303, 0.00970722373939169, 0.0149378350960501,
        0.0223794710636485, 0.032698732726609, 0.0466682080548466, 0.0651555334325362,
        0.0891031392409415, 0.119497411288696, 0.157326203484366, 0.203523998858602,
