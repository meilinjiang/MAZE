...
negQ_G1.zilonm <- function(dat, theta, K, num_Z, Z_names, B = NULL,
    ...
    # betas, delta, alpha0k, alpha1k, alphaZ_k, xi0, w0k, gammas,
    # eta
    dat_g1 <- dat[which(dat$Mobs > 0), ]
    ...
    if (!calculate_tau) {
        ...
        l1_ik <- -log(theta_trans[["delta"]]) + log(1 - pf0) - log(M_group1) -
            log(2 * pi) - theta_trans[["xi0"]] - (log(M_group1) - theta_trans[["mu_ik"]])^2/(2 *
            ...
            theta_trans[["beta5"]] * X_group1 * M_group1 - theta_trans[["beta_T_Z"]])^2/(2 *
            theta_trans[["delta"]]^2)
...
ComputeInit2.zilonm <- function(dat, K, num_Z, Z_names, XMint, B, limits,
    explicit = T) {
...
effects.zilonm <- function(dat, theta, x1, x2, K, num_Z, zval, XMint,
    ...
    if (Group1) {
        ...
        # num_Z + (2 + num_Z) * K + 2 + K - 1)], -Inf, 0, rep(0,
        # num_Z), Inf)
    }
