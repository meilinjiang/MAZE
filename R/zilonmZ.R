# Mediation analysis for zero-inflated log-normal
    ...
    if (explicit) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            m <- constrOptim(init, function(x) negQ2_G1(dat,
                x, K, num_Z, Z_names, XMint, B, tau), grad = NULL,
...
negQ_G2.zilonm <- function(dat, theta, K, num_Z, Z_names, B,
    ...
    LogInt_k <- loghicpp_all(X_group2, Y_group2, theta_trans[["mu_ik"]],
        theta_trans[["sig"]], Ypart, theta_trans[["beta1"]],
        theta_trans[["beta5"]], theta_trans[["delta"]], theta_trans[["eta"]],
        nodes, weights, B)
    ...
    if (calculate_se == T) {
        ...
        for (var in seq_len(2 + num_Z)) {
            g_NIE1_alpha_k[var, ] <- (theta_trans[["beta1"]] +
                ...
                diff((1 - Del_x12) * exp_x12_k * desginMat[,
                  var])
        ...
        g_NDE <- c(rep(0, 3), diff(x12), if (XMint[1]) diff(x12) *
            ...
            diff(x12) * (1 - Del_x12[1]) * (exp_x12_k[1, 1:(K -
            1)] - exp_x12_k[1, K]), g_NDE_gammas, 0)
        # beta3, (beta4,beta5)
...
nodes <- list(c(0.999999999999957, 0.999999988875665, 0.999977477192462,
    ...
    0), c(0.999999999952856, 0.999999204737115, 0.999688264028353,
...
weights <- list(c(1.35817842745391e-12, 2.1431204556943e-07,
    ...
    1.13827224337631, 1.24670120745186, 1.34527888476625, 1.4300083548723,
    1.49722622254104, 1.54388111617696, 1.56778143130722), c(2.18359220992336e-12,
    ...
    0.197262203197437, 0.209929530480208, 0.223177394922185,
    0.237016458319419, 0.25145648308451, 0.266506245563135,
    ...
    0.669733055410291, 0.696510795146547, 0.72369968702683,
    0.751265924524357, 0.779173199704798, 0.807382723018763,
    0.835853256308267, 0.864541159619669, 0.893400452347196,
    0.922382889152455, 0.951438051016353, 0.980513451680855,
    1.00955465962943, 1.03850543563739, 1.0673078857975, 1.09590262979297,
    1.1242289840506, 1.15222515926255, 1.17982847161733, 1.20697556693131,
    1.23360265672195, 1.25964576511667, 1.28504098534673, 1.30972474443744,
    1.33363407457576, 1.35670688951561, 1.37888226427314, 1.40010071626944,
    1.42030448599969, 1.43943781524641, 1.45744722081255, 1.47428176172808,
    1.4898932978833, 1.50423673806368, 1.51727027540505, 1.52895560835458,
    1.53925814531188, 1.54814719123556, 1.55559611463166, 1.56158249349181,
