...
compare_mu.zilonm <- function(dat, init2, group, K, num_Z,
    Z_names, B = NULL) {
    if (group == 1) {
...
bounds.zilonm <- function(dat, K, group, num_Z) {
    ...
    if (K == 1) {
        if (group == 1) {
            ...
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, 3 +
                num_Z))
        } else {
        ...
        if (group == 1) {
            ...
            ui <- rbind(ui1, ui2[s + 2 + (2 + num_Z) * K +
                ...
                1)))
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 +
                num_Z) * K + 1), rep(1e-06, K - 1), rep(-1000,
...
ComputeInit2.zilonm <- function(dat, K, num_Z, Z_names,
    ...
    if (explicit) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            alpha_k <- matrix(init[4 + num_Z + 1:((2 + num_Z) *
                K)], ncol = K)
            desginMat_M <- cbind(1, dat_g1[, c("X", Z_names)])
            for (var in 1:nrow(alpha_k)) {
                var_k <- matrix(rep(desginMat_M[, var],
                  K), ncol = K)
                alpha_k[var, ] <- colSums(tau * var_k *
                  (log(M_group1) - apply(alpha_k[-var, 1:K,
                    drop = F], 2, function(t) rowSums(k_to_ik(t,
                    n1) * desginMat_M[, -var]))))/colSums(tau *
                  var_k^2)
            }
            sig2 <- sum(tau * (log(M_group1) - apply(alpha_k,
                2, function(t) rowSums(k_to_ik(t, n1) *
                  desginMat_M)))^2)/n1
            xi0 <- log(sqrt(sig2))
            # beta02 ,beta1, beta34, beta_Z
...
negQ_G2.zilonm <- function(dat, theta, K, num_Z, Z_names,
    ...
    LogInt_k <- loghicpp_all(X_group2, Y_group2, theta_trans[["mu_ik"]],
        ...
        theta_trans[["delta"]], theta_trans[["eta"]], nodes,
        weights, B)

    l2_ik <- cbind(-log(theta_trans[["delta"]]) - (Y_group2 -
        theta_trans[["beta0"]] - theta_trans[["beta3"]] *
    ...
    if (calculate_se == T) {
        ...
        for (var in seq_len(2 + num_Z)) {
            g_NIE1_alpha_k[var, ] <- theta_trans[["beta1"]] *
                theta_trans[["psi_k"]] * diff((1 - Del_x12) *
                exp_x12 * desginMat[, var])
            g_NIE1_gammas[var] <- theta_trans[["beta1"]] *
                (-diff(Del_x12 * (1 - Del_x12) * m_x12 *
                  desginMat[, var]))
            g_NIE2_gammas[var] <- (theta_trans[["beta2"]] +
                theta_trans[["beta4"]] * x2) * (-diff(Del_x12 *
                (1 - Del_x12) * desginMat[, var]))
        }
        # beta1, alpha0_k, alpha1_k, xi0, psi_k-1,
        # gamma0, gamma1
        g_NIE1 <- c(diff((1 - Del_x12) * m_x12), g_NIE1_alpha_k,
            sig2 * NIE1, theta_trans[["beta1"]] * diff((exp_x12[,
                1:(K - 1)] - exp_x12[, K]) * (1 - Del_x12)),
            g_NIE1_gammas)
        if (K == 1) {
        ...
        g_NIE <- c(g_NIE1[1], g_NIE2[1:2], g_NIE1[1 + 1:((3 +
            num_Z) * K)], g_NIE1[(3 + num_Z) * K + 1 + 1:(2 +
            num_Z)] + g_NIE2[2 + 1:(2 + num_Z)])

        if (Group1) {
            g_NIE1 <- g_NIE <- g_NIE1[1:((3 + num_Z) * K +
                1)]
            V1 <- V3 <- vcovar[c(2, 4 + num_Z + 1:((3 +
                num_Z) * K)), c(2, 4 + num_Z + 1:((3 + num_Z) *
                K))]
            NIE2_se <- NA
        } else {
            V1 <- vcovar[c(2, 6 + num_Z + 1:((3 + num_Z) *
                K + 2 + num_Z)), c(2, 6 + num_Z + 1:((3 +
                num_Z) * K + 2 + num_Z))]
            V2 <- vcovar[c(3, 5, 6 + num_Z + (3 + num_Z) *
                K + 1:(2 + num_Z)), c(3, 5, 6 + num_Z +
                (3 + num_Z) * K + 1:(2 + num_Z))]
            V3 <- vcovar[c(2, 3, 5, 6 + num_Z + 1:((3 +
                num_Z) * K + 2 + num_Z)), c(2, 3, 5, 6 +
                num_Z + 1:((3 + num_Z) * K + 2 + num_Z))]
...
parameter_names.zilonm <- function(dat, K, num_Z, Z_names) {
    c(paste0("beta", c(0:4, Z_names)), "delta", paste0("alpha",
        rep(1:K, each = 2 + num_Z), rep(c(0:1, Z_names),
            K)), "xi0", paste0("psi", 1:K)[-K], paste0("gamma",
        c(0:1, Z_names)), "eta")
