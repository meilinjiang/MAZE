...
negQ_G1.zilonm <- function(dat, theta, K, num_Z, Z_names, B = NULL,
    ...
    if (!calculate_tau) {
        ...
        l1_ik <- -log(theta_trans[["delta"]]) + log(1 - pf0) -
            log(M_group1) - log(2 * pi) - theta_trans[["xi0"]] -
            (log(M_group1) - theta_trans[["mu_ik"]])^2/(2 * theta_trans[["sig"]]^2) -
...
ComputeInit2.zilonm <- function(dat, K, num_Z, Z_names, XMint,
    B, limits, explicit = T) {
...
negQ_G2.zilonm <- function(dat, theta, K, num_Z, Z_names, B, tauG2 = NULL,
    ...
    Ypart <- theta_trans[["beta0"]] + theta_trans[["beta2"]] +
        (theta_trans[["beta3"]] + theta_trans[["beta4"]]) * X_group2 +
        theta_trans[["beta_T_Z"]]
...
effects.zilonm <- function(dat, theta, x1, x2, K, num_Z, zval,
    ...
    NIE2 <- (theta_trans[["beta2"]] + theta_trans[["beta4"]] *
        x2) * (-diff(Del_x12))
    NIE <- NIE1 + NIE2
    NDE <- diff(x12) * (theta_trans[["beta3"]] + (1 - Del_x12[1]) *
        (theta_trans[["beta4"]] + theta_trans[["beta5"]] * m_x12[1]))
    ...
    if (calculate_se == T) {
        ...
        g_NIE1_alpha_k <- g_NDE_alpha_k <- matrix(NA, 2 + num_Z,
            K)
        ...
        for (var in seq_len(2 + num_Z)) {
            ...
            g_NIE1_gammas[var] <- (theta_trans[["beta1"]] + theta_trans[["beta5"]] *
                x2) * (-diff(Del_x12 * (1 - Del_x12) * m_x12 *
                desginMat[, var]))
            g_NIE2_gammas[var] <- (theta_trans[["beta2"]] + theta_trans[["beta4"]] *
                x2) * (-diff(Del_x12 * (1 - Del_x12) * desginMat[,
                var]))
            g_NDE_alpha_k[var, ] <- theta_trans[["beta5"]] * diff(x12) *
                (1 - Del_x12[1]) * theta_trans[["psi_k"]] * exp_x12[1,
                ] * desginMat[, var]

            g_NDE_gammas[var] <- diff(x12) * (theta_trans[["beta4"]] +
                theta_trans[["beta5"]] * m_x12[1]) * (-1) * Del_x12[1] *
                (1 - Del_x12[1]) * desginMat[, var]
        }
        # beta1, (beta5), alpha0_k, alpha1_k, xi0, psi_k-1,
        # gammas
        g_NIE1 <- c(0, diff((1 - Del_x12) * m_x12), rep(0, 2 +
            XMint[1]), if (XMint[2]) x2 * diff((1 - Del_x12) *
            m_x12) else NULL, rep(0, num_Z + 1), g_NIE1_alpha_k,
            sig2 * NIE1, if (K == 1) NULL else (theta_trans[["beta1"]] +
                theta_trans[["beta5"]] * x2) * diff((exp_x12[,
                1:(K - 1)] - exp_x12[, K]) * (1 - Del_x12)), g_NIE1_gammas,
            0)
        ...
        # beta3, (beta4,beta5), alphas_k, xi0, psi_k-1,
        # gammas
        g_NDE <- c(rep(0, 3), diff(x12), if (XMint[1]) diff(x12) *
            (1 - Del_x12[1]) else NULL, if (XMint[2]) diff(x12) *
            (1 - Del_x12[1]) * m_x12[1] else NULL, rep(0, num_Z +
            1), g_NDE_alpha_k, theta_trans[["beta5"]] * diff(x12) *
            (1 - Del_x12[1]) * m_x12[1] * sig2, if (K == 1) NULL else theta_trans[["beta5"]] *
            diff(x12) * (1 - Del_x12[1]) * (exp_x12[1, 1:(K - 1)] -
            exp_x12[1, K]), g_NDE_gammas, 0)
        # beta3, (beta4,beta5)
...
nodes <- list(c(0.999999999999957, 0.999999988875665, 0.999977477192462,
    ...
    c(0.99999999999993, 0.999999999999817, 0.999999999999537, 0.999999999998861,
        ...
        0.9705755717919, 0.968117008926856, 0.96549077410362, 0.962688471739078,
...
weights <- list(c(1.35817842745391e-12, 2.1431204556943e-07, 0.000266200513752717,
    ...
    c(4.93785387766319e-11, 1.86872822687364e-08, 1.82633205937107e-06,
        ...
        0.737437848361548, 1.19346302584916, 1.52328371863471),
    c(8.6759314149796e-12, 2.52163479185301e-10, 4.87600609742406e-09,
        6.58351851271834e-08, 6.47775660359297e-07, 4.82371820326155e-06,
        2.81101643279401e-05, 0.0001320523412561, 0.000513393824067903,
        0.00169087399814264, 0.00481629814392846, 0.012083543599158,
        0.027133510013712, 0.0552896837422406, 0.103432154223333,
        0.179324412110728, 0.290240679312454, 0.440833236273858,
        0.630405135164744, 0.85017285645662, 1.08163498549007,
        1.29747575042498, 1.46601442671697, 1.55877335553333),
    c(3.48419376702611e-12, 2.09893354045115e-11, 1.13060553474947e-10,
        5.4828357797095e-10, 2.40917732564759e-09, 9.64988889610896e-09,
        3.54347771714219e-08, 1.19924427829028e-07, 3.75954118623606e-07,
        1.09688351259013e-06, 2.99166158781388e-06, 7.65957585252031e-06,
        1.84818135998792e-05, 4.21831838417576e-05, 9.13908174907101e-05,
        0.000188564429767003, 0.000371666936216778, 0.000701859515684242,
        0.00127332794470824, 0.00222508270647864, 0.00375425097743183,
        0.00613003763208303, 0.00970722373939169, 0.0149378350960501,
        0.0223794710636485, 0.032698732726609, 0.0466682080548466,
        0.0651555334325362, 0.0891031392409415, 0.119497411288696,
        0.157326203484366, 0.203523998858602, 0.258904639514054,
        0.324082539611529, 0.399384741525717, 0.484758091214755,
        0.579678103087788, 0.683068516344264, 0.793242700820517,
