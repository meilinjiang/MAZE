# Mediation analysis for zero-inflated log-normal mediators
# with confounders
...
trans.zilonm <- function(dat, theta, K, xval, num_Z, zval) {
    ...
    theta_trans <- list(beta0 = beta0, beta1 = beta1, beta2 = beta2,
        ...
        alpha_k = alpha_k, psi_k = psi_k, eta = eta, beta_T_Z = beta_T_Z,
        mu_ik = mu_ik, xi0 = xi0, sig = sig, psi_ik = psi_ik, Del_i = Del_i)
    return(theta_trans)
}
...
negQ_G1.zilonm <- function(dat, theta, K, num_Z, Z_names, B = NULL,
    tauG1 = NULL, calculate_tau = F, calculate_ll = F) {
    # betas, delta, alpha0k, alpha1k, alphaZ_k, xi0, w0k,
    # gammas, eta
    dat_g1 <- dat[which(dat$Mobs > 0), ]
    ...
    if (!calculate_tau) {
        ...
        l1_ik <- -log(theta_trans[["delta"]]) + log(1 - pf0) - log(M_group1) -
            log(2 * pi) - theta_trans[["xi0"]] - (log(M_group1) -
    ...
    if (K == 1) {
        ...
        if (group == 1) {
            ...
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 + num_Z) *
                K + 1), rep(1e-06, K - 1), rep(-(1 - 1e-06), K -
                1), -(1 - 1e-06))
        } else {
            ...
            ui <- rbind(ui1, ui2[s + 2 + (2 + num_Z) * K + 1:(K -
                ...
                K - 1), rep(0, (2 + num_Z) + 1)))
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 + num_Z) *
                K + 1), rep(1e-06, K - 1), rep(-1000, 2 + num_Z),
...
ComputeInit2.zilonm <- function(dat, K, num_Z, Z_names, XMint, B,
    limits, explicit = T) {
    ...
    if (explicit) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            alpha_k <- matrix(init[s + 1 + 1:((2 + num_Z) * K)],
                ncol = K)
            ...
            sig2 <- sum(tau * (log(M_group1) - apply(alpha_k, 2,
                function(t) rowSums(k_to_ik(t, n1) * desginMat_M)))^2)/n1
            xi0 <- log(sqrt(sig2))
            # beta02 ,beta1, beta34, beta5, beta_Z
            betas <- init[1:s]
            desginMat_Y <- cbind(1, dat_g1[, c("Mobs", "X")], dat_g1$X *
                dat_g1$Mobs, dat_g1[, Z_names])
            if (!XMint[2]) {
...
negQ_G2.zilonm <- function(dat, theta, K, num_Z, Z_names, B, tauG2 = NULL,
    ...
    LogInt_k <- loghicpp_all(X_group2, Y_group2, theta_trans[["mu_ik"]],
        theta_trans[["sig"]], Ypart, theta_trans[["beta1"]], theta_trans[["beta5"]],
...
effects.zilonm <- function(dat, theta, x1, x2, K, num_Z, zval, XMint,
    calculate_se = F, vcovar = NULL, Group1 = F) {
    if (Group1) {
        # theta <- c(theta[1:2], 0, theta[3], 0, theta[3 + 1:(1
        # + num_Z + (2 + num_Z) * K + 2 + K - 1)], -Inf, 0,
        # rep(0, num_Z), Inf)
    }
    ...
    if (calculate_se == T) {
        ...
        for (var in seq_len(2 + num_Z)) {
            g_NIE1_alpha_k[var, ] <- (theta_trans[["beta1"]] + theta_trans[["beta5"]] *
                x2) * theta_trans[["psi_k"]] * diff((1 - Del_x12) *
                exp_x12 * desginMat[, var])
            g_NIE1_gammas[var] <- (theta_trans[["beta1"]] + theta_trans[["beta5"]] *
                x2) * (-diff(Del_x12 * (1 - Del_x12) * m_x12 * desginMat[,
                var]))
            g_NIE2_gammas[var] <- (theta_trans[["beta2"]] + theta_trans[["beta4"]] *
                x2) * (-diff(Del_x12 * (1 - Del_x12) * desginMat[,
                var]))
        }
        # beta1, (beta5), alpha0_k, alpha1_k, xi0, psi_k-1,
        # gammas
        g_NIE1 <- c(0, diff((1 - Del_x12) * m_x12), rep(0, 2 + XMint[1]),
            if (XMint[2]) x2 * diff((1 - Del_x12) * m_x12) else NULL,
            rep(0, num_Z + 1), g_NIE1_alpha_k, sig2 * NIE1, if (K ==
                1) NULL else (theta_trans[["beta1"]] + theta_trans[["beta5"]] *
                x2) * diff((exp_x12[, 1:(K - 1)] - exp_x12[, K]) *
                (1 - Del_x12)), g_NIE1_gammas, 0)

        # beta2, (beta4), gammas
        g_NIE2 <- c(rep(0, 2), -diff(Del_x12), 0, if (XMint[1]) x2 *
            (-diff(Del_x12)) else NULL, rep(0, XMint[2] + num_Z +
            1 + (2 + num_Z) * K + 1 + K - 1), g_NIE2_gammas, 0)
        g_NIE <- g_NIE1 + g_NIE2
...
nodes <- list(c(0.999999999999957, 0.999999988875665, 0.999977477192462,
    0.997514856457224, 0.951367964072747, 0.674271492248436, 0),
    c(0.999999999952856, 0.999999204737115, 0.999688264028353, 0.987040560507377,
        0.859569058689897, 0.377209738164034), c(0.999999999998232,
        0.999999999142705, 0.999999892781612, 0.99999531604122, 0.999909384695144,
        0.999065196455786, 0.994055506631402, 0.973966868195677,
        0.914879263264575, 0.7806074389832, 0.539146705387968, 0.194357003324935),
    c(0.999999999999708, 0.999999999990394, 0.999999999789733, 0.999999996787199,
        ...
        0.935160857521985, 0.88989140278426, 0.823317005506402, 0.731018034792561,
        ...
        0.99987486504878, 0.999767971599561, 0.999584750351518, 0.999281111921792,
        ...
        0.802798741343241, 0.75669390863373, 0.703550005147142, 0.643176758985205,
        ...
        0.999999860371215, 0.999999766023332, 0.99999961398855, 0.999999372707335,
        0.99999899541069, 0.999998413810965, 0.999997529623805, 0.999996203347166,
        ...
        0.9996398313456, 0.999522237651217, 0.999371401140938, 0.999179448934886,
        ...
        0.90909831816302, 0.896614254280076, 0.882834988244669, 0.867683175775646,
        0.851084007987849, 0.832966293919411, 0.813263608502974,
        0.791915492376142, 0.768868686768246, 0.744078383547347,
        0.717509467487324, 0.689137725061668, 0.65895099174335, 0.626950208051043,
        0.593150353591953, 0.557581228260778, 0.52028805069123, 0.481331846116905,
        ...
        0.999999999488718, 0.999999999277422, 0.99999999898421, 0.999999998579458,
        ...
        0.999412633574952, 0.99932760616283, 0.999231775767899, 0.99912397635239,
        ...
        0.9958915954671, 0.995425695705623, 0.994914629802639, 0.994354840195921,
