...
DataGen_call.zilonm <- function(dat_placeholder, theta, K, num_Z, n, B,
    x1, x2, zval) {
    X <- rnorm(n)
    ...
    Y <- theta_trans[["beta0"]] + theta_trans[["beta1"]] * M + theta_trans[["beta2"]] *
        ind + theta_trans[["beta3"]] * X + theta_trans[["beta4"]] * X *
        ind + eps
...
DataGen_call.zinbm <- function(dat_placeholder, theta, K, num_Z, n, B, x1,
    x2, zval) {
    X <- rnorm(n)
    theta_trans <- trans(dat_placeholder, theta, K, xval = X, num_Z, zval = NULL)
    eps <- rnorm(n, 0, theta_trans[["delta"]])
    ...
    # mu=rowSums(L_ik*theta_trans[['mu_ik']])) NB could also generate
    # 0's ind_nb = 0: M is an excessive zero
    M[ind_nb == 0] <- 0
    # 1(M > 0)
    ind <- (M > 0) * 1
...
DataGen_call.zipm <- function(dat_placeholder, theta, K, num_Z, n, B, x1,
    x2, zval) {
    X <- rnorm(n)
    theta_trans <- trans(dat_placeholder, theta, K, xval = X, num_Z, zval = NULL)
    eps <- rnorm(n, 0, theta_trans[["delta"]])
    ...
    # M <- rpois(n,lambda=rowSums(L_ik*theta_trans[['lambda_ik']])) #
    # poisson could also generate 0's ind_poi = 0: M is an excessive
    # zero
    M[ind_poi == 0] <- 0
    # 1(M > 0)
    ind <- (M > 0) * 1
