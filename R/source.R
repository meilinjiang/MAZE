...
# out <- logitmodel$coefficients[, 'Estimate'] } return(out)
# #c(gamma0,gamma1) }
...
G1_init <- function(dat, init, K, num_Z, Z_names, XMint, initials_for_full = F) {
    ...
    # zilonm, with beta5 initials <- c(init[1:2], 0, init[3], 0,
    # init[4], init[4 + 1:(num_Z + (2 + num_Z) * K + 2 + K - 1)],
    # gammas, 0.01)

    # zinbm, no beta5 initials <- c(init[1:2], 0, init[3], 0,
    # init[3 + 1:(num_Z + (2 + num_Z) * K + 2 + K - 1)], gammas,
    # 0.01)

    # zipm, no beta5 initials <- c(init[1:2], 0, init[3], 0,
    # init[3 + 1:(num_Z + (2 + num_Z) * K + 1 + K - 1)], gammas,
    # 0.01)
    return(initials)
}
...
# calculate_tau, calculate_ll)) } } return(out) } # for
# calculating observed I (EM approximation) g <- function(dat, x,
# y, K, B) { tau_f <- negQ(dat, y, K, B, calculate_tau = T) out <-
...
parameter_names <- function(dat, K, num_Z, Z_names, XMint) {
    ...
    out <- c(paste0("beta", c(0:5, Z_names)), "delta", paste0("alpha",
        ...
        paste0("psi", 1:K)[-K], paste0("gamma", c(0:1, Z_names)), "eta")
    if (sum(XMint) != 2) {
        out <- out[-c(5, 6)[!XMint]]
    }
    return(out)
...
G1_index <- function(dat, K, num_Z, XMint) {
    # c(1:2, 4, 6, 6 + 1:(2 + num_Z + (2 + num_Z) * K + (K - 1)))
    c(1:2, 4, (4 + XMint[1] + XMint[2]) * XMint[2], 4 + XMint[1] + XMint[2] +
        1:(num_Z + 1 + (2 + num_Z) * K + ifelse("zipm" %in% class(dat),
            0, 1) + (K - 1)))
}
...
# calculate_tau = T) p <- length(init2) countEM <- NA } else {
# tau2 <- negQ(dat, init2, K, B, calculate_tau = T) p <-
# length(init2) init <- rep(0, p) countEM <- 0 bd <- bounds(dat,
# K, group = 2) # M-step while (euclidean_dist(init, init2) >
...
# switch$tau2 countEM <- countEM + 1 } } est <- init2 n <-
# dim(dat)[1] negll <- negQ(dat, est, K, B, calculate_ll = T) AIC
# <- 2 * negll + 2 * p BIC <- 2 * negll + p * log(n)
# return(list(init = cinit, est = est, countEM = countEM, AIC =
# AIC, BIC = BIC, tau2 = tau2)) }

analysis <- function(dat, K, num_Z, Z_names, XMint, B, limits, seed) {
    cinit <- init2 <- ComputeInit2(dat, K, num_Z, Z_names, XMint, B,
        limits, explicit = T)
    # cinit <- init2 <- true2
    if (sum(dat$Mobs == 0) == 0) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            m <- constrOptim(init, function(x) negQ(dat, x, K, num_Z,
                Z_names, XMint, B, tau), grad = NULL, ui = bd$ui, ci = bd$ci,
...
# analysis2 <- function(dat, K, B, est, tau2, x1, x2) { # method
# 1: hessian from log-likelihood function, # parameter estimation
...
# numDeriv::grad(function(x) g(dat, x, y, K, B), x = est) } h.2 <-
# jacobian(g2, est) hess <- h.1 + h.2 vcovar <- solve(hess) var <-
# diag(vcovar) if (sum(var < 0) > 0) { print('All methods var <
# 0.') } } } se <- sqrt(var) # mediation effects Group1 <-
