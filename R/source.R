...
G1_init <- function(dat, init, K, num_Z, Z_names,
    ...
    if (initials_for_full) {
        # to set initials for parameters in
        # G2 for full analysis
        initials <- c(init[1:2], 0, init[3], rep(0,
            XMint[1] * 1), init[4:length(init)],
            gammas, 0.01)
    } else {
        # to set parameters not estimating
        # in G1 at fixed values
        initials <- c(init[1:2], 0, init[3], rep(0,
    ...
    # init[4 + 1:(num_Z + (2 + num_Z) * K +
    # 2 + K - 1)], gammas, 0.01)

    # zinbm, no beta5 initials <-
    # c(init[1:2], 0, init[3], 0, init[3 +
    # 1:(num_Z + (2 + num_Z) * K + 2 + K -
    # 1)], gammas, 0.01)

    # zipm, no beta5 initials <-
    # c(init[1:2], 0, init[3], 0, init[3 +
    # 1:(num_Z + (2 + num_Z) * K + 1 + K -
    # 1)], gammas, 0.01)
    return(initials)
}
...
G12_init <- function(theta, XMint) {
    theta <- c(theta[1:4], rep(0, !XMint[1]),
        theta[5:length(theta)])
    theta <- c(theta[1:5], rep(0, !XMint[2]),
        theta[6:length(theta)])
    return(theta)
}

# Q for all
negQ <- function(dat, theta, K, num_Z, Z_names,
    XMint, B, tau = NULL, calculate_tau = F, calculate_ll = F) {
    if (sum(dat$Mobs == 0) > 0) {
        # fix parameter not estimating in
        # full analysis (group12)
        theta <- G12_init(theta, XMint)
        if (!calculate_tau) {
            if (!calculate_ll) {
                out <- negQ_G1(dat, theta, K,
            ...
            out <- list(tauG1 = negQ_G1(dat, theta,
                ...
                calculate_tau, calculate_ll),
                tauG2 = negQ_G2(dat, theta, K,
                  num_Z, Z_names, B, tauG2 = NULL,
                  calculate_tau, calculate_ll))
        }
        # if (is.nan(out)) {
        # print(theta);print(out) }
        ...
        if (!calculate_tau) {
            if (!calculate_ll) {
                out <- negQ2_G1(dat, theta, K,
                  num_Z, Z_names, XMint, B, tauG1 = tau$tauG1,
                  calculate_tau, calculate_ll)
            } else {
                out <- negQ2_G1(dat, theta, K,
                  num_Z, Z_names, XMint, B, tauG1 = NULL,
                  calculate_tau, calculate_ll)
            }
            # print(theta);print(out)
        } else {
            out <- list(tauG1 = negQ2_G1(dat,
                theta, K, num_Z, Z_names, XMint,
                B = NULL, tauG1 = NULL, calculate_tau,
                calculate_ll))
        }
    }
    return(out)
...
parameter_names <- function(dat, K, num_Z, Z_names,
    ...
    variation <- if ("zipm" %in% class(dat))
        ...
        "r")
    out <- c(paste0("beta", c(0:5, Z_names)),
        "delta", paste0("alpha", rep(1:K, each = 2 +
            num_Z), rep(c(0:1, Z_names), K)),
        variation, paste0("psi", 1:K)[-K], paste0("gamma",
            c(0:1, Z_names)), "eta")
    if (sum(XMint) != 2) {
        out <- out[-c(5, 6)[!XMint]]
    }
    ...
    if (sum(dat$Mobs == 0) == 0) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            m <- constrOptim(init, function(x) negQ(dat,
                ...
                tau), grad = NULL, ui = bd$ui,
                ci = bd$ci, outer.iterations = 500,
            ...
            if (m$convergence == 0) {
                ...
                print(paste0("dist = ", class(dat)[1],
                  ", K = ", K, ", seed = ", seed,
                  ", analysis()_constrOptim() = ",
                  m$convergence, ", countEM = ",
                  countEM))
                print(m)
...
analysis2 <- function(dat, K, B, est, tau2, x1,
    x2, num_Z, Z_names, XMint, zval, mval) {
    ...
    # negQ(dat, x, K, num_Z, Z_names, XMint,
    ...
    if (sum(var < 0) > 0) {
        ...
        g2 <- function(y) {
            numDeriv::grad(function(x) g(dat,
                x, y, K, num_Z, Z_names, XMint,
                B), x = est)
        }
    ...
    MedZIM <- effects(dat, theta = est, x1, x2,
        K, num_Z, zval, mval, XMint, calculate_se = T,
        vcovar, Group1)
    out <- list(se = se, MedZIM = MedZIM)
    return(out)
}

realanalysis <- function(dat, distM_sequence,
    K_sequence, selection, XMint, x1, x2, zval,
    ...
    fun <- c("analysis", "bounds", "compare_mu",
        ...
        "euclidean_dist", "expit", "k_to_ik",
        "loghicpp_all", "logbinom", "loghik_zinbm",
        "loghik_zipm", "negQ", "negQ_G1", "negQ_G2",
        "negQ2_G1", "trans")
    ...
    models <- foreach(i = seq_len(niter), .multicombine = T,
        ...
        .combine = "list", .errorhandling = "pass") %dopar%
        {
            ...
            model_i <- tryCatch({
                analysis(dat2, K = iter$K[i],
                  num_Z, Z_names, XMint, B, limits,
                  seed)
            }, error = function(e) {
                print(paste0("dist = ", iter$distM[i],
                  ", K = ", iter$K[i], ", analysis()_error = ",
                  e))
    ...
    analysis2_out <- tryCatch({
        ...
        print(paste0("dist = ", selected_dist,
            ", K = ", selected_K, ", analysis2()_error = ",
            e))
