...
# calculate_tau = F, calculate_ll = F) { if (sum(dat$Mobs == 0) > 0) {
# if (!calculate_tau) { if (!calculate_ll) { out <- negQ_G1(dat,
# theta, K, B, tauG1 = tau$tauG1, calculate_tau, calculate_ll) +
# negQ_G2(dat, theta, K, B, tauG2 = tau$tauG2, calculate_tau,
# calculate_ll) } else { out <- negQ_G1(dat, theta, K, B, tauG1 =
# NULL, calculate_tau, calculate_ll) + negQ_G2(dat, theta, K, B, tauG2
# = NULL, calculate_tau, calculate_ll) } # print(theta);print(out) if
# (is.nan(out)) # { print(theta) print(out) } } else { out <-
# list(tauG1 = negQ_G1(dat, theta, K, B = NULL, tauG1 = NULL,
# calculate_tau, calculate_ll), tauG2 = negQ_G2(dat, theta, K, B,
# tauG2 = NULL, calculate_tau, calculate_ll)) } } else { if
# (!calculate_tau) { if (!calculate_ll) { out <- negQ2_G1(dat, theta,
# K, B, tauG1 = tau$tauG1, calculate_tau, calculate_ll) } else { out
# <- negQ2_G1(dat, theta, K, B, tauG1 = NULL, calculate_tau,
# calculate_ll) } # print(theta);print(out) } else { out <- list(tauG1
# = negQ2_G1(dat, theta, K, B = NULL, tauG1 = NULL, calculate_tau,
# calculate_ll)) } } return(out) } # for calculating observed I (EM
# approximation) g <- function(dat, x, y, K, B) { tau_f <- negQ(dat,
# y, K, B, calculate_tau = T) out <- negQ(dat, x, K, B, tau_f)
# return(out) }
        ...
        if (!calculate_tau) {
            if (!calculate_ll) {
                out <- negQ_G1(dat, theta, K, num_Z, Z_names, B, tauG1 = tau$tauG1,
                  calculate_tau, calculate_ll) + negQ_G2(dat, theta, K,
                  num_Z, Z_names, B, tauG2 = tau$tauG2, calculate_tau, calculate_ll)
            } else {
                out <- negQ_G1(dat, theta, K, num_Z, Z_names, B, tauG1 = NULL,
                  calculate_tau, calculate_ll) + negQ_G2(dat, theta, K,
                  num_Z, Z_names, B, tauG2 = NULL, calculate_tau, calculate_ll)
            }
            # print(theta);print(out)
        } else {
            out <- list(tauG1 = negQ_G1(dat, theta, K, num_Z, Z_names, B = NULL,
                tauG1 = NULL, calculate_tau, calculate_ll), tauG2 = negQ_G2(dat,
...
# (sum(dat$Mobs == 0) == 0) { index <- G1_index(dat, K) cinit <- init2
# <- init2[index] tau2 <- negQ(dat, init2, K, B, calculate_tau = T) p
# <- length(init2) countEM <- NA } else { tau2 <- negQ(dat, init2, K,
# B, calculate_tau = T) p <- length(init2) init <- rep(0, p) countEM
# <- 0 bd <- bounds(dat, K, group = 2) # M-step while
# (euclidean_dist(init, init2) > limits) { init <- init2 tau <- tau2 #
# m2 <- # nlminb(init,function(x)negQ(dat,x,K,B,tau), # lower =
...
# negQ(dat, est, K, B, calculate_ll = T) AIC <- 2 * negll + 2 * p BIC
# <- 2 * negll + p * log(n) return(list(init = cinit, est = est,
# countEM = countEM, AIC = AIC, BIC = BIC, tau2 = tau2)) }

analysis <- function(dat, K, num_Z, Z_names, B, limits, seed) {
    cinit <- init2 <- ComputeInit2(dat, K, num_Z, Z_names, B, limits, explicit = T)
    # cinit <- init2 <- true2
    if (sum(dat$Mobs == 0) == 0) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            m <- constrOptim(init, function(x) negQ(dat, x, K, num_Z, Z_names,
...
# hessian from log-likelihood function, # parameter estimation cannot
# do this hess <- pracma::hessian(function(x) negQ(dat, x, K, B,
# calculate_ll = T), x = est) vcovar <- solve(hess) var <-
# diag(vcovar) if (sum(var < 0) > 0) { hess <-
# numDeriv::hessian(function(x) negQ(dat, x, K, B, calculate_ll = T),
# x = est) vcovar <- solve(hess) var <- diag(vcovar) if (sum(var < 0)
# > 0) { # method 2: hessian matrix from EM # approximation h.1 <-
# pracma::hessian(function(x) negQ(dat, x, K, B, tau2), x = est) g2 <-
# function(y) { numDeriv::grad(function(x) g(dat, x, y, K, B), x =
# est) } h.2 <- jacobian(g2, est) hess <- h.1 + h.2 vcovar <-
# solve(hess) var <- diag(vcovar) if (sum(var < 0) > 0) { print('All
# methods var < 0.') } } } se <- sqrt(var) # mediation effects Group1
# <- sum(dat$Mobs == 0) == 0 MedZIM <- effects(dat, est, x1, x2, K,
# calculate_se = T, vcovar, Group1) return(list(se = se, MedZIM =
# MedZIM)) }
...
analysis2 <- function(dat, K, B, est, tau2, x1, x2, num_Z, Z_names, zval) {
    ...
    var <- diag(vcovar)
    if (sum(var < 0) > 0) {
        hess <- numDeriv::hessian(function(x) negQ(dat, x, K, num_Z, Z_names,
            B, calculate_ll = T), x = est)
        vcovar <- solve(hess)
        var <- diag(vcovar)
        if (sum(var < 0) > 0) {
            # method 2: hessian matrix from EM approximation
            h.1 <- pracma::hessian(function(x) negQ(dat, x, K, num_Z, Z_names,
                B, tau2), x = est)
            g2 <- function(y) {
                grad(function(x) g(dat, x, y, K, num_Z, Z_names, B), x = est)
            }
            h.2 <- jacobian(g2, est)
            hess <- h.1 + h.2
            vcovar <- solve(hess)
...
realanalysis <- function(dat, distM_sequence, K_sequence, selection, x1,
    x2, zval, num_Z, Z_names, limits, B, seed, ncore) {
    set.seed(seed)
    ...
    fun <- c("analysis", "bounds", "compare_mu", "ComputeInit", "ComputeInit2",
        ...
        "logbinom", "loghik_zinbm", "loghik_zipm", "negQ", "negQ_G1", "negQ_G2",
        "negQ2_G1", "trans")
    parallel::clusterExport(cl = cl, varlist = c("nodes", "weights", fun),
    ...
    models <- foreach(i = seq_len(niter), .multicombine = T, .packages = c("stats",
        "flexmix", "MASS"), .combine = "list", .errorhandling = "pass") %dopar%
        {
            ...
            model_i <- tryCatch({
                analysis(dat2, K = iter$K[i], num_Z, Z_names, B, limits,
                  seed)
            }, error = function(e) {
                # print(e)
                list(init = NA, est = NA, countEM = NA, AIC = Inf, BIC = Inf,
                  tau2 = NA, e = e)
            })
    ...
    # t1 <- Sys.time() out <- analysis2(dat, K = selected_K, B, est =
    # selected_model$est, tau2 = selected_model$tau2, x1, x2, num_Z,
    # Z_names, zval)
