...
G1_init <- function(dat, init, K, num_Z, Z_names, XMint, initials_for_full = F) {
    ...
    if (initials_for_full) {
        ...
        # analysis
        initials <- c(init[1:2], 0, init[3], rep(0, XMint[1] *
            1), init[4:length(init)], gammas, 0.01)
    } else {
...
negQ <- function(dat, theta, K, num_Z, Z_names, XMint, B, tau = NULL,
    ...
    if (sum(dat$Mobs == 0) > 0) {
        ...
        if (!calculate_tau) {
            ...
            out <- list(tauG1 = negQ2_G1(dat, theta, K, num_Z,
                Z_names, XMint, B = NULL, tauG1 = NULL, calculate_tau,
                calculate_ll))
        }
    }
    return(out)
...
analysis <- function(dat, K, num_Z, Z_names, XMint, B, limits,
    seed) {
    ...
    if (sum(dat$Mobs == 0) == 0) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            if (m$convergence == 0) {
                ...
                print(paste0("dist = ", class(dat)[1], ", K = ",
                  K, ", seed = ", seed, ", analysis()_constrOptim() = ",
                  m$convergence, ", countEM = ", countEM))
                print(m)
...
analysis2 <- function(dat, K, B, est, tau2, x1, x2, num_Z, Z_names,
    ...
    # average effects: E(NIE) = E[ E(NIE|Z=zval) ] = 1/n
    # \sum_{z \in Data} E(NIE|Z=zval) Var(NIE) = E[
    # Var(NIE|Z=zval) ] + Var[ E(NIE|Z=zval) ]
    if (num_Z == 0) {
        MedZIM_avg <- MedZIM
    } else {
        n <- dim(dat)[1]
        ...
        for (i in seq_len(n)) {
            MedZIM_i <- effects(dat, theta = est, x1, x2, K,
                num_Z, zval = dat[i, Z_names], mval, XMint,
...
realanalysis <- function(dat, distM_sequence, K_sequence, selection,
    ...
    models <- foreach(i = seq_len(niter), .multicombine = T,
        ...
        model_i <- tryCatch({
            ...
            print(paste0("dist = ", iter$distM[i], ", K = ",
                iter$K[i], ", analysis()_error = ", e))
    ...
    analysis2_out <- tryCatch({
        ...
        print(paste0("dist = ", selected_dist, ", K = ", selected_K,
            ", analysis2()_error = ", e))
        list(se = NA, MedZIM = data.frame(eff = rep(NA, 5),
            eff_se = rep(NA, 5)), e = e)
    })
    ...
    rownames(results_effects) <- c(paste0(effects_names, "_cond"),
        paste0(effects_names, "_avg"))
