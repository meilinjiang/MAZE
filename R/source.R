...
# gammas_init <- function(dat) { if (sum(dat$Mobs == 0) == 0) {
# out <- c(-Inf, 0) } else { ind_Del <- (dat$Mobs == 0) * 1
# logitmodel <- summary(glm(ind_Del ~ dat$X, family =
# 'binomial')) out <- logitmodel$coefficients[, 'Estimate'] }
...
gammas_init <- function(dat, num_Z = 0, Z_names = NULL) {
    if (sum(dat$Mobs == 0) == 0) {
        ...
        fm <- as.formula(paste0("ind_Del~", paste0(c("X", Z_names),
            collapse = "+"), collapse = ""))
...
# negative expectation of log-likelihood function with respect
# to conditional distribution of 1(Ci = k) given data and
...
G1_init <- function(dat, init, K, num_Z, Z_names, XMint, initials_for_full = F) {
    ...
    if (initials_for_full) {
        # to set initials for parameters in G2 for full
        # analysis
        initials <- c(init[1:2], 0, init[3], rep(0, XMint[1] * 1),
            init[4:length(init)], gammas, 0.01)
    } else {
        # to set parameters not estimating in G1 at fixed
        # values
...
compare_mu <- function(dat, init2, group, K, num_Z, Z_names, XMint,
    ...
    theta_trans <- trans(dat, initials, K, xval = mean(dat2$X), num_Z,
        zval)
...
# calculate_tau, calculate_ll) } else { out <- negQ_G1(dat,
# theta, K, B, tauG1 = NULL, calculate_tau, calculate_ll) +
# negQ_G2(dat, theta, K, B, tauG2 = NULL, calculate_tau,
# calculate_ll) } # print(theta);print(out) if (is.nan(out)) #
# { print(theta) print(out) } } else { out <- list(tauG1 =
# negQ_G1(dat, theta, K, B = NULL, tauG1 = NULL, calculate_tau,
# calculate_ll), tauG2 = negQ_G2(dat, theta, K, B, tauG2 =
# NULL, calculate_tau, calculate_ll)) } } else { if
# (!calculate_tau) { if (!calculate_ll) { out <- negQ2_G1(dat,
# theta, K, B, tauG1 = tau$tauG1, calculate_tau, calculate_ll)
# } else { out <- negQ2_G1(dat, theta, K, B, tauG1 = NULL,
# calculate_tau, calculate_ll) } # print(theta);print(out) }
# else { out <- list(tauG1 = negQ2_G1(dat, theta, K, B = NULL,
# tauG1 = NULL, calculate_tau, calculate_ll)) } } return(out) }
# # for calculating observed I (EM approximation) g <-
# function(dat, x, y, K, B) { tau_f <- negQ(dat, y, K, B,
# calculate_tau = T) out <- negQ(dat, x, K, B, tau_f)
# return(out) }

# Q for all
negQ <- function(dat, theta, K, num_Z, Z_names, XMint, B, tau = NULL,
    ...
    if (sum(dat$Mobs == 0) > 0) {
        ...
        if (!calculate_tau) {
            if (!calculate_ll) {
                ...
                out <- negQ_G1(dat, theta, K, num_Z, Z_names, B,
                  tauG1 = NULL, calculate_tau, calculate_ll) + negQ_G2(dat,
                  theta, K, num_Z, Z_names, B, tauG2 = NULL, calculate_tau,
                  calculate_ll)
            }
            ...
            out <- list(tauG1 = negQ_G1(dat, theta, K, num_Z, Z_names,
                ...
                tauG2 = negQ_G2(dat, theta, K, num_Z, Z_names, B,
        ...
        if (!calculate_tau) {
            if (!calculate_ll) {
                out <- negQ2_G1(dat, theta, K, num_Z, Z_names, XMint,
            ...
            out <- list(tauG1 = negQ2_G1(dat, theta, K, num_Z, Z_names,
                XMint, B = NULL, tauG1 = NULL, calculate_tau, calculate_ll))
        }
    }
    return(out)
}
...
G1_index <- function(dat, K, num_Z, XMint) {
    # c(1:2, 4, 6, 6 + 1:(2 + num_Z + (2 + num_Z) * K + (K -
    # 1)))
    c(1:2, 4, (4 + XMint[1] + XMint[2]) * XMint[2], 4 + XMint[1] +
        XMint[2] + 1:(num_Z + 1 + (2 + num_Z) * K + ifelse("zipm" %in%
        class(dat), 0, 1) + (K - 1)))
}

# analysis <- function(dat, K, B, limits, seed) { cinit <-
# init2 <- ComputeInit2(dat, K, B, limits) # cinit <- init2 <-
# true2 if (sum(dat$Mobs == 0) == 0) { index <- G1_index(dat,
# K) cinit <- init2 <- init2[index] tau2 <- negQ(dat, init2, K,
# B, calculate_tau = T) p <- length(init2) countEM <- NA } else
# { tau2 <- negQ(dat, init2, K, B, calculate_tau = T) p <-
# length(init2) init <- rep(0, p) countEM <- 0 bd <-
# bounds(dat, K, group = 2) # M-step while
# (euclidean_dist(init, init2) > limits) { init <- init2 tau <-
# tau2 # m2 <- # nlminb(init,function(x)negQ(dat,x,K,B,tau), #
...
# function(x) negQ(dat, x, K, B, tau), grad = NULL, ui = bd$ui,
# ci = bd$ci, outer.iterations = 500, control = list(maxit =
# 50000)) #
# negQ(dat,true,K,B,negQ(dat,true,K,B,calculate_tau=T)) if
# (m$convergence != 0) { print(paste0('seed=', seed, ',
# countEM=', countEM, ', convergence=', m$convergence))
# print(m) init2 <- NA break } else { init2 <- m$par } switch
# <- compare_mu(dat, init2, group = 2, dat$X, K, B) init2 <-
# switch$init2 tau2 <- switch$tau2 countEM <- countEM + 1 } }
# est <- init2 n <- dim(dat)[1] negll <- negQ(dat, est, K, B,
# calculate_ll = T) AIC <- 2 * negll + 2 * p BIC <- 2 * negll +
# p * log(n) return(list(init = cinit, est = est, countEM =
# countEM, AIC = AIC, BIC = BIC, tau2 = tau2)) }

analysis <- function(dat, K, num_Z, Z_names, XMint, B, limits, seed) {
    cinit <- init2 <- ComputeInit2(dat, K, num_Z, Z_names, XMint,
        B, limits, explicit = T)
    # cinit <- init2 <- true2
    if (sum(dat$Mobs == 0) == 0) {
        ...
        while (euclidean_dist(init, init2) > limits) {
            ...
            m <- constrOptim(init, function(x) negQ(dat, x, K, num_Z,
                Z_names, XMint, B, tau), grad = NULL, ui = bd$ui,
            ...
            if (m$convergence != 0) {
                print(paste0("seed=", seed, ", countEM=", countEM,
            ...
            switch <- compare_mu(dat, init2, group = 2, K, num_Z,
                Z_names, XMint, B)
            init2 <- switch$init2
            tau2 <- switch$tau2
            countEM <- countEM + 1
...
# estimation cannot do this hess <- pracma::hessian(function(x)
# negQ(dat, x, K, B, calculate_ll = T), x = est) vcovar <-
# solve(hess) var <- diag(vcovar) if (sum(var < 0) > 0) { hess
# <- numDeriv::hessian(function(x) negQ(dat, x, K, B,
# calculate_ll = T), x = est) vcovar <- solve(hess) var <-
# diag(vcovar) if (sum(var < 0) > 0) { # method 2: hessian
# matrix from EM # approximation h.1 <-
# pracma::hessian(function(x) negQ(dat, x, K, B, tau2), x =
# est) g2 <- function(y) { numDeriv::grad(function(x) g(dat, x,
# y, K, B), x = est) } h.2 <- jacobian(g2, est) hess <- h.1 +
# h.2 vcovar <- solve(hess) var <- diag(vcovar) if (sum(var <
# 0) > 0) { print('All methods var < 0.') } } } se <- sqrt(var)
# # mediation effects Group1 <- sum(dat$Mobs == 0) == 0 MedZIM
# <- effects(dat, est, x1, x2, K, calculate_se = T, vcovar,
# Group1) return(list(se = se, MedZIM = MedZIM)) }
...
analysis2 <- function(dat, K, B, est, tau2, x1, x2, num_Z, Z_names,
    ...
    MedZIM <- effects(dat, theta = est, x1, x2, K, num_Z, zval, XMint,
        calculate_se = T, vcovar, Group1)
...
realanalysis <- function(dat, distM_sequence, K_sequence, selection,
    ...
    fun <- c("analysis", "bounds", "compare_mu", "ComputeInit", "ComputeInit2",
        "gammas_init", "euclidean_dist", "expit", "k_to_ik", "loghicpp_all",
        "logbinom", "loghik_zinbm", "loghik_zipm", "negQ", "negQ_G1",
        "negQ_G2", "negQ2_G1", "trans")
    parallel::clusterExport(cl = cl, varlist = c("nodes", "weights",
        fun), envir = parent.env(environment()))
    ...
    # t1 <- Sys.time() out <- analysis2(dat, K = selected_K, B,
    # est = selected_model$est, tau2 = selected_model$tau2, x1,
    # x2, num_Z, Z_names, XMint, zval)
    analysis2_out <- tryCatch({
        analysis2(dat, K = selected_K, B, est = selected_model$est,
            tau2 = selected_model$tau2, x1, x2, num_Z, Z_names, XMint,
            zval)
    }, error = function(e) {
        print(e)
    ...
    out <- list(results_effects = results_effects, results_parameters = results_parameters,
        ...
        AIC = AICs[which.min(selection_criteria)], models = models,
        analysis2_out = analysis2_out)
    return(out)
}
