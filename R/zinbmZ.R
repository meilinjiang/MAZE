# Mediation analysis for zero-inflated negative binomial
# mediators with confounders
...
trans.zinbm <- function(dat, theta, K, xval, num_Z, zval) {
    ...
    if (K == 1) {
        ...
        psi_k <- c(theta[8 + num_Z + (2 + num_Z) * K + 1:(K - 1)],
            1 - sum(theta[8 + num_Z + (2 + num_Z) * K + 1:(K - 1)]))
    }
    ...
    theta_trans <- list(beta0 = beta0, beta1 = beta1, beta2 = beta2,
        ...
        alpha_k = alpha_k, r = r, psi_k = psi_k, gammas = gammas,
        eta = eta, beta_T_Z = beta_T_Z, mu_ik = mu_ik, Delstar_i = Delstar_i,
...
ComputeInit.zinbm <- function(dat, K, num_Z, Z_names, XMint) {
    ...
    # flexmix(fm_M, data = dat_g1, k=K, model = FLXMRnegbin())
...
negQ_G1.zinbm <- function(dat, theta, K, num_Z, Z_names, B = NULL,
    ...
    if (!calculate_tau) {
        ...
        if (!calculate_ll) {
            ...
            # estimation cannot use it (has many saddle points
            # so need EM), but hessian can l1_i <- log(
    ...
    if (K == 1) {
        ...
        if (group == 1) {
            ...
            ui <- rbind(ui1, ui2[s + 2 + (2 + num_Z) * K + 1:(K -
                ...
                K - 1)))
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 + num_Z) *
                K), 1e-06, rep(1e-06, K - 1), rep(-(1 - 1e-06), K -
                1), -(1 - 1e-06))
        } else {
...
negQ_G2.zinbm <- function(dat, theta, K, num_Z, Z_names, B, tauG2 = NULL,
    ...
    for (i in 1:B) {
        loghik_m[[i]] <- loghik_zinbm(m = i, theta_trans[["r"]],
            ...
            theta_trans[["beta5"]], theta_trans[["beta_T_Z"]], theta_trans[["delta"]],
    ...
    if (!calculate_tau) {
        ...
        if (!calculate_ll) {
            ...
            # hessian: from log likelihood function parameter
            # estimation cannot use it (has many saddle points
            # so need EM), but hessian can l2_i <- log(
...
f_NIE2_zinbm <- function(dat, theta, K, num_Z, XMint, x12, z12) {
    ...
    theta_trans <- trans.zinbm(dat, theta, K, xval = x12, num_Z,
        zval = z12)
...
effects.zinbm <- function(dat, theta, x1, x2, K, num_Z, zval, XMint,
    ...
    if (calculate_se == T) {
        ...
        g_NIE1 <- c(0, diff((1 - Delstar_x12) * m_x12), rep(0, 2 +
            XMint[1]), if (XMint[2]) x2 * diff((1 - Delstar_x12) *
            m_x12) else NULL, rep(0, num_Z + 1), g_NIE1_alpha_k,
            0, if (K == 1) NULL else (theta_trans[["beta1"]] + theta_trans[["beta5"]] *
                x2) * diff((theta_trans[["mu_ik"]][, 1:(K - 1)] -
                theta_trans[["mu_ik"]][, K]) * (1 - Delstar_x12)),
            g_NIE1_gammas, 0)

        # beta2, (beta4), alpha0_k, alpha1_k, r, psi_k-1,
        # gammas
        g_NIE2 <- numDeriv::grad(function(x) f_NIE2_zinbm(dat, x,
