# Mediation analysis for zero-inflated negative binomial mediators
# with confounders
...
trans.zinbm <- function(dat, theta, K, xval, num_Z, zval) {
    ...
    if (K == 1) {
        ...
        psi_k <- c(theta[8 + num_Z + (2 + num_Z) * K + 1:(K - 1)], 1 -
            sum(theta[8 + num_Z + (2 + num_Z) * K + 1:(K - 1)]))
    }
    ...
    theta_trans <- list(beta0 = beta0, beta1 = beta1, beta2 = beta2,
        ...
        alpha_k = alpha_k, r = r, psi_k = psi_k, gammas = gammas, eta = eta,
        beta_T_Z = beta_T_Z, mu_ik = mu_ik, Delstar_i = Delstar_i, p_ik = p_ik,
        psi_ik = psi_ik, Del_i = Del_i)
    return(theta_trans)
}
...
ComputeInit.zinbm <- function(dat, K, num_Z, Z_names, XMint) {
    ...
    fm_M <- as.formula(paste0("log(Mobs)~", paste0(c("X", Z_names),
        collapse = "+"), collapse = ""))
    for (j in seq_len(K)) {
...
negQ_G1.zinbm <- function(dat, theta, K, num_Z, Z_names, B = NULL, tauG1 = NULL,
    ...
    # betas, delta, alpha0k, alpha1k, alphaZ_k, r, w0k, gammas,
    # eta
    dat_g1 <- dat[which(dat$Mobs > 0), ]
    ...
    if (!calculate_tau) {
        ...
        l1_ik <- -log(theta_trans[["delta"]]) - 0.5 * log(2 * pi) +
            log(1 - pf0) - (Y_group1 - theta_trans[["beta0"]] - theta_trans[["beta1"]] *
    ...
    if (K == 1) {
        ...
        if (group == 1) {
            ...
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 + num_Z) * K),
                1e-06, rep(1e-06, K - 1), rep(-(1 - 1e-06), K - 1),
                -(1 - 1e-06))
        } else {
...
ComputeInit2.zinbm <- function(dat, K, num_Z, Z_names, XMint, B, limits,
    explicit = T) {
...
loghik_zinbm <- function(m, r, p, beta0, beta1, beta2, beta3, beta4,
    ...
    logbinom(r, m) + m * log(1 - p) - ((y - beta0 - beta1 * m - beta2 -
        (beta3 + beta4) * x - beta5 * x * m - beta_T_Z)^2)/(2 * delta^2) -
        eta^2 * m
}
...
f_NIE2_zinbm <- function(dat, theta, K, num_Z, XMint, x12, z12) {
    ...
    NIE2 <- (theta_trans[["beta2"]] + theta_trans[["beta4"]] * x12[2]) *
        (-diff(theta_trans[["Del_i"]]))
    return(NIE2)
}

effects.zinbm <- function(dat, theta, x1, x2, K, num_Z, zval, XMint,
    ...
    NIE1 <- (theta_trans[["beta1"]] + theta_trans[["beta5"]] * x2) *
        diff((1 - Delstar_x12) * m_x12)
    ...
    if (calculate_se == T) {
        ...
        g_NIE1 <- c(0, diff((1 - Delstar_x12) * m_x12), rep(0, 2 + XMint[1]),
            if (XMint[2]) x2 * diff((1 - Delstar_x12) * m_x12) else NULL,
            rep(0, num_Z + 1), g_NIE1_alpha_k, 0, if (K == 1) NULL else (theta_trans[["beta1"]] +
                theta_trans[["beta5"]] * x2) * diff((theta_trans[["mu_ik"]][,
                1:(K - 1)] - theta_trans[["mu_ik"]][, K]) * (1 - Delstar_x12)),
            g_NIE1_gammas, 0)

        # beta2, (beta4), alpha0_k, alpha1_k, r, psi_k-1, gammas
        g_NIE2 <- numDeriv::grad(function(x) f_NIE2_zinbm(dat, x, K,
            num_Z, XMint, x12, z12), theta)
        if (sum(XMint) != 2) {
