...
trans.zinbm <- function(dat, theta, K, xval, num_Z, zval) {
    ...
    beta4 <- theta[5]
    delta <- theta[6 + num_Z]
    alpha_k <- theta[6 + num_Z + 1:((2 + num_Z) * K)]
    r <- theta[7 + num_Z + (2 + num_Z) * K]
    if (K == 1) {
        psi_k <- 1
    } else {
    ...
    theta_trans <- list(beta0 = beta0, beta1 = beta1, beta2 = beta2, beta3 = beta3,
        beta4 = beta4, delta = delta, alpha_k = alpha_k, r = r, psi_k = psi_k,
        gammas = gammas, eta = eta, beta_T_Z = beta_T_Z, mu_ik = mu_ik,
        Delstar_i = Delstar_i, p_ik = p_ik, psi_ik = psi_ik, Del_i = Del_i)
    return(theta_trans)
}

ComputeInit.zinbm <- function(dat, K, num_Z, Z_names) {
    # group 1, flexmix
    dat_g1 <- dat[which(dat$Mobs > 0), ]

    # fit <- summary(lm(Y_group1 ~ M_group1 + X_group1))
    fm_Y <- as.formula(paste0("Y~", paste0(c("Mobs", "X", Z_names), collapse = "+"),
        collapse = ""))
    fit <- summary(lm(fm_Y, data = dat_g1))
    # beta02, beta1, beta34, beta_Z
    betas <- fit$coefficients[, "Estimate"]
    delta <- fit$sigma

    # fm_M <- as.formula(paste0('Mobs~', paste0(c('X', Z_names),
    # collapse = '+'), collapse = '')) cl <- flexmix(fm_M, data =
    # dat_g1, k=K, model = FLXMRnegbin())
    ...
    for (j in seq_len(K)) {
        ...
        m_j <- dat_g1_j$Mobs
        # r_init <- mean(m_j)^2/(var(m_j)-mean(m_j)) # numerator
        # larger, denominator smaller than true
        r_init <- mean(m_j)/2
        fit2 <- lm(fm_M, dat_g1_j)
        para <- cbind(para, c(coef(fit2), r_init))
    }

    # method 3: distribution 1 as the one with smaller cluster mean
    # since x=age, intercepts are not accurate enough
    mu <- cl$centers
...
# estimates for group 1: -Q1
negQ_G1.zinbm <- function(dat, theta, K, num_Z, Z_names, B = NULL, tauG1 = NULL,
    calculate_tau = F, calculate_ll = F) {
    # beta0-5, alpha0k, alpha1k, xi0, w0k, gamma0, gamma1, eta, delta
    dat_g1 <- dat[which(dat$Mobs > 0), ]
    ...
    log_dnb_nz <- logbinom(theta_trans[["r"]], M_group1) + M_group1 * log(1 -
        theta_trans[["p_ik"]]) - log(theta_trans[["p_ik"]]^(-theta_trans[["r"]]) -
        1)
    # group 1
    if (!calculate_tau) {
        ...
        pf0[M_group1 > B] <- 0
        l1_ik <- -log(theta_trans[["delta"]]) - 0.5 * log(2 * pi) + log(1 -
            pf0) - (Y_group1 - theta_trans[["beta0"]] - theta_trans[["beta1"]] *
        ...
        if (!calculate_ll) {
            ...
            # estimation cannot use it (has many saddle points so need
            # EM), but hessian can l1_i <- log( bigpsi1*exp(l1_ik1) +
            # bigpsi2*exp(l1_ik2) )
        ...
        # calculate conditional expectation of 1(Ci = k) given data
...
G1_init.zinbm <- function(dat, init, K, num_Z, Z_names) {
    gammas <- gammas_init(dat, num_Z, Z_names)
    initials <- c(init[1:2], 0, init[3], 0, init[3 + 1:(num_Z + (2 + num_Z) *
        K + 2 + K - 1)], gammas, 0.01)
    return(initials)
}

compare_mu.zinbm <- function(dat, init2, group, K, num_Z, Z_names, B = NULL) {
    if (group == 1) {
    ...
    ord <- order(theta_trans[["mu_ik"]])
    if (sum(ord != 1:K) > 0) {
        init2[s + 1 + 1:((2 + num_Z) * K)] <- c(matrix(theta_trans[["alpha_k"]],
            ncol = K)[, ord])
        init2[s + 2 + (2 + num_Z) * K + 1:(K - 1)] <- theta_trans[["psi_k"]][ord][-K]
    }
    if (group == 1) {
        tau2 <- negQ2_G1(dat, init2, K, num_Z, Z_names, calculate_tau = T)
    } else {
...
bounds.zinbm <- function(dat, K, group, num_Z) {
    # r need to be > 0
    s <- ifelse(group == 1, 3, 5) + num_Z
    # e <- ifelse(group==1,0,3)
    ul <- 1000
    if (K == 1) {
        if (group == 1) {
            ...
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, 2 + num_Z), 1e-06,
                rep(-1000, 2 + num_Z), 1e-06)
        }
    } else {
        if (group == 1) {
            ui1 <- diag(rep(1, s + 2 + (2 + num_Z) * K + (K - 1)))
            ui2 <- diag(c(rep(0, s + 2 + (2 + num_Z) * K), rep(-1, K - 1)))
            ui <- rbind(ui1, ui2[s + 2 + (2 + num_Z) * K + 1:(K - 1), ],
                c(rep(0, s + 2 + (2 + num_Z) * K), rep(-1, K - 1)))
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 + num_Z) * K), 1e-06,
                rep(1e-06, K - 1), rep(-(1 - 1e-06), K - 1), -(1 - 1e-06))
        } else {
            ui1 <- diag(rep(1, s + 3 + (2 + num_Z) * K + (K - 1) + 2 + num_Z))
            ui2 <- diag(c(rep(0, s + 2 + (2 + num_Z) * K), rep(-1, K - 1),
                rep(0, (2 + num_Z) + 1)))
            ui <- rbind(ui1, ui2[s + 2 + (2 + num_Z) * K + 1:(K - 1), ],
                c(rep(0, s + 2 + (2 + num_Z) * K), rep(-1, K - 1), rep(0,
                  (2 + num_Z) + 1)))
            ci <- c(rep(-1000, s), 1e-06, rep(-1000, (2 + num_Z) * K), 1e-06,
                rep(1e-06, K - 1), rep(-1000, 2 + num_Z), 1e-06, rep(-(1 -
                  1e-06), K - 1), -(1 - 1e-06))
        }
    }
...
ComputeInit2.zinbm <- function(dat, K, num_Z, Z_names, B, limits, explicit = T) {
    init2 <- ComputeInit(dat, K, num_Z, Z_names)
    tau2 <- negQ2_G1(dat, init2, K, num_Z, Z_names, calculate_tau = T)
    init <- rep(0, length(init2))
    countEM <- 0
    X_group1 <- dat$X[which(dat$Mobs > 0)]
    bd <- bounds(dat, K, group = 1, num_Z)
    # M-step
    while (euclidean_dist(init, init2) > limits) {
        ...
        # m <-
        # nlminb(init,function(x)negQ2_G1(dat,x,K,num_Z,Z_names,B,tau),lower
        # = c(rep(-1000,9),1e-6))
    ...
    # beta0,beta1,beta3,beta5, alpha_0k, alpha_1k, xi_0, w_0k, delta
    initials <- G1_init(dat, init2, K, num_Z, Z_names)
    initials[5 + num_Z + 1 + (2 + num_Z) * K + 1] <- initials[5 + num_Z +
        1 + (2 + num_Z) * K + 1]/2
...
loghik_zinbm <- function(m, r, p, beta0, beta1, beta2, beta3, beta4, beta_T_Z,
    delta, eta, x, y) {
    logbinom(r, m) + m * log(1 - p) - ((y - beta0 - (beta1 * m) - beta2 -
        (beta3 + beta4) * x - beta_T_Z)^2)/(2 * delta^2) - eta^2 * m
}
...
negQ_G2.zinbm <- function(dat, theta, K, num_Z, Z_names, B, tauG2 = NULL,
    ...
    for (i in 1:B) {
        loghik_m[[i]] <- loghik_zinbm(m = i, theta_trans[["r"]], theta_trans[["p_ik"]],
            ...
            theta_trans[["beta3"]], theta_trans[["beta4"]], theta_trans[["beta_T_Z"]],
            theta_trans[["delta"]], theta_trans[["eta"]], x = X_group2,
            y = Y_group2)
    }
    loghik_mmax <- NULL
    for (k in seq_len(K)) {
        loghik_mmax <- cbind(loghik_mmax, apply(sapply(loghik_m, function(t) t[,
            k]), 1, max))
    }
    ...
    l2_ik <- cbind(-(Y_group2 - theta_trans[["beta0"]] - theta_trans[["beta3"]] *
        ...
        -log(theta_trans[["p_ik"]]^(-theta_trans[["r"]]) - 1) + output) -
        log(theta_trans[["delta"]]) - 0.5 * log(2 * pi)
    ...
    if (!calculate_tau) {
        ...
        if (!calculate_ll) {
            ...
            # hessian: from log likelihood function parameter
            # estimation cannot use it (has many saddle points so need
            # EM), but hessian can l2_i <- log( bigpsi0*exp(l2_ik0) +
...
f_NIE2_zinbm <- function(t, K, num_Z, x12, z12) {
    beta2 <- t[1]
    beta4 <- t[2]
    alpha_k <- t[2 + 1:((2 + num_Z) * K)]
    r <- t[3 + (2 + num_Z) * K]
    if (K == 1) {
        psi_k <- 1
    } else {
        psi_k <- c(t[3 + (2 + num_Z) * K + 1:(K - 1)], 1 - sum(t[3 + (2 +
            num_Z) * K + 1:(K - 1)]))
    }
    gammas <- t[3 + (2 + num_Z) * K + K - 1 + 1:(2 + num_Z)]

    designMat_M <- cbind(1, x12, z12)
    n0 <- 2
    logmu_ik <- apply(matrix(alpha_k, ncol = K), 2, function(t) rowSums(k_to_ik(t,
        n0) * designMat_M))
    mu_ik <- exp(logmu_ik)
    ...
    p_ik <- r/(r + mu_ik)
    Del_i <- Delstar_i + (1 - Delstar_i) * rowSums(psi_ik * p_ik^r)
    out <- (beta2 + beta4 * x12[2]) * (-diff(Del_i))
    return(out)
}

effects.zinbm <- function(dat, theta, x1, x2, K, num_Z, zval, calculate_se = F,
    ...
    # f_NIE2(t, K, num_Z, x12, z12) NDE <- (x2 -
    ...
    if (calculate_se == T) {
        ...
        for (var in seq_len(2 + num_Z)) {
            g_NIE1_alpha_k[var, ] <- theta_trans[["beta1"]] * theta_trans[["psi_k"]] *
                diff((1 - Delstar_x12) * theta_trans[["mu_ik"]] * desginMat[,
                  var])
            g_NIE1_gammas[var] <- theta_trans[["beta1"]] * (-1) * diff(Delstar_x12 *
                (1 - Delstar_x12) * m_x12 * desginMat[, var])
        }
        # alpha0_k, alpha1_k, psi_k-1, gamma0, gamma1
        g_NIE1 <- c(diff((1 - Delstar_x12) * m_x12), g_NIE1_alpha_k, theta_trans[["beta1"]] *
            diff((theta_trans[["mu_ik"]][, 1:(K - 1)] - theta_trans[["mu_ik"]][,
                K]) * (1 - Delstar_x12)), g_NIE1_gammas)
        if (K == 1) {
            g_NIE1 <- g_NIE1[-(2 + num_Z + 2)]
        }

        t <- c(theta_trans[["beta2"]], theta_trans[["beta4"]], theta_trans[["alpha_k"]],
            theta_trans[["r"]], theta_trans[["psi_k"]][-K], theta_trans[["gammas"]])
        g_NIE2 <- numDeriv::grad(function(x) f_NIE2_zinbm(x, K, num_Z, x12,
            z12), t)

        # beta1 # beta2,4, alpha0_k, alpha1_k,r psi_k-1, gammas
        g_NIE <- c(g_NIE1[1], g_NIE2[1:2], g_NIE1[1 + 1:((2 + num_Z) * K)] +
            g_NIE2[2 + 1:((2 + num_Z) * K)], g_NIE2[3 + (2 + num_Z) * K],
            g_NIE1[1 + (2 + num_Z) * K + 1:(K - 1 + 2 + num_Z)] + g_NIE2[2 +
                (2 + num_Z) * K + 1 + 1:(K - 1 + 2 + num_Z)])

        V1 <- vcovar[c(2, 6 + num_Z + 1:((2 + num_Z) * K), 7 + num_Z + (2 +
            num_Z) * K + 1:(K - 1 + 2 + num_Z)), c(2, 6 + num_Z + 1:((2 +
            num_Z) * K), 7 + num_Z + (2 + num_Z) * K + 1:(K - 1 + 2 + num_Z))]
        V2 <- vcovar[c(3, 5, 6 + num_Z + 1:((2 + num_Z) * K + 1 + K - 1 +
            2 + num_Z)), c(3, 5, 6 + num_Z + 1:((2 + num_Z) * K + 1 + K -
            1 + 2 + num_Z))]
        V3 <- vcovar[c(2, 3, 5, 6 + num_Z + 1:((3 + num_Z) * K + 2 + num_Z)),
            c(2, 3, 5, 6 + num_Z + 1:((3 + num_Z) * K + 2 + num_Z))]
...
parameter_names.zinbm <- function(dat, K, num_Z, Z_names) {
    c(paste0("beta", c(0:4, Z_names)), "delta", paste0("alpha", rep(1:K,
        each = 2 + num_Z), rep(c(0:1, Z_names), K)), "r", paste0("psi",
        1:K)[-K], paste0("gamma", c(0:1, Z_names)), "eta")
}

G1_index.zinbm <- function(dat, K, num_Z) {
    c(1:2, 4, 5 + 1:(2 + num_Z + (2 + num_Z) * K + (K - 1)))
}
