...
# negative binomial mediators with
# confounders
...
trans.zinbm <- function(dat, theta, K, xval, num_Z,
    ...
    if (K == 1) {
        ...
        psi_k <- c(theta[8 + num_Z + (2 + num_Z) *
            K + 1:(K - 1)], 1 - sum(theta[8 +
            num_Z + (2 + num_Z) * K + 1:(K - 1)]))
    }
    gammas <- theta[8 + num_Z + (2 + num_Z) *
        K + (K - 1) + 1:(2 + num_Z)]
    eta <- theta[9 + num_Z + (2 + num_Z) * K +
        (K - 1) + (2 + num_Z)]
    ...
    theta_trans <- list(beta0 = beta0, beta1 = beta1,
        ...
        r = r, psi_k = psi_k, gammas = gammas,
        eta = eta, beta_T_Z = beta_T_Z, mu_ik = mu_ik,
        Delstar_i = Delstar_i, p_ik = p_ik, psi_ik = psi_ik,
        Del_i = Del_i)
    return(theta_trans)
}
...
negQ_G1.zinbm <- function(dat, theta, K, num_Z,
    ...
    theta_trans <- trans(dat, theta, K, X_group1,
        num_Z, Z_group1)
    log_dnb_nz <- logbinom(theta_trans[["r"]],
        M_group1) + M_group1 * log(1 - theta_trans[["p_ik"]]) -
        log(theta_trans[["p_ik"]]^(-theta_trans[["r"]]) -
            1)
    # group 1
    if (!calculate_tau) {
    ...
    if (K == 1) {
        ...
        if (group == 1) {
            ui1 <- diag(rep(1, s + 2 + (2 + num_Z) *
                K + (K - 1)))
            ui2 <- diag(c(rep(0, s + 2 + (2 +
                num_Z) * K), rep(-1, K - 1)))
            ui <- rbind(ui1, ui2[s + 2 + (2 +
...
ComputeInit2.zinbm <- function(dat, K, num_Z,
    ...
    # beta_Z, delta, alpha_0k, alpha_1k, r,
    # w_0k
...
f_NIE2_zinbm <- function(dat, theta, K, num_Z,
    XMint, x12, z12) {
    # theta <- G12_init(theta, XMint)
    theta_trans <- trans.zinbm(dat, theta, K,
        xval = x12, num_Z, zval = z12)
    NIE2 <- (theta_trans[["beta2"]] + theta_trans[["beta4"]] *
        x12[2]) * (-diff(theta_trans[["Del_i"]]))
    return(NIE2)
}

f_NDE_zinbm <- function(dat, theta, K, num_Z,
    XMint, x12, z12) {
    # theta <- G12_init(theta, XMint)
    theta_trans <- trans.zinbm(dat, theta, K,
        xval = x12, num_Z, zval = z12)
    NDE <- diff(x12) * (theta_trans[["beta3"]] +
        theta_trans[["beta4"]] * (1 - theta_trans[["Del_i"]][1]) +
        theta_trans[["beta5"]] * (1 - theta_trans[["Delstar_i"]][1]) *
            rowSums(theta_trans[["psi_ik"]] *
                theta_trans[["mu_ik"]])[1])
    return(NDE)
}

effects.zinbm <- function(dat, theta, x1, x2,
    K, num_Z, zval, mval, XMint, calculate_se = F,
    ...
    NIE2 <- (theta_trans[["beta2"]] + theta_trans[["beta4"]] *
        x2) * (-diff(Del_x12))
    # f_NIE2_zinbm(dat, theta, K, num_Z,
    # XMint, x12, z12)
    NIE <- NIE1 + NIE2
    NDE <- diff(x12) * (theta_trans[["beta3"]] +
